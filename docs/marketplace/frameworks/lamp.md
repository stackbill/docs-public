...
### Once the LAMP is deployed

 You can view the LAMP instance immediately by visiting the instance’s IP address in your browser.
...
Setting up an SSL certificate enables HTTPS on the web server, which secures the traffic between the server and the clients connecting to it. Certbot is a free and automated way to set up SSL certificates on a server. It’s included as part of the LAMP deployment to make securing the domain easier.
